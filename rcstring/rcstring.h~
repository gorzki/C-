...
public:
	...
	bool isNumericChar(rcstring & ,unsigned int i);
	int atoi();
	rcstring& toLower();
...
inline void rcstring::write(unsigned int i, char c)
{
  ...
  bool rcstring::isNumericChar(rcstring & x, unsigned int i)
  {
      return (x.data->s[i] >= '0' && x.data->s[i] <= '9')? true: false;
	}
  int rcstring::atoi()
  {
    if ((*this).data == NULL)
    ...
    else if((*this).data->s[i] =='+')
      i++;
    for (; (*this).data->s[i] != '\0'; ++i)
    {
        if (isNumericChar((*this),i) == false)
            break;
